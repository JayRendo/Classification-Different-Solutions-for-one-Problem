...
model <- glm(Species ~., data = data_train, family = "binomial")
summary(model)

#Figuring out the accuracy on the test model
#The test set is fairly balanced so this is an 
#accurate measure
y_pred <- predict(model, newdata = data_test[,1:4], type = "response")
count <- 0
for (i in 1:nrow(data_test)){
  if (((y_pred[i] <= 0.5) && (data_test$Species[i] == "versicolor")) || (y_pred[i] > 0.5 & data_test$Species[i] == "virginica")){
    count = count + 1
  }
}
